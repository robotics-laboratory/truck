...
def add_arguments(parser: argparse.ArgumentParser):
    parser.add_argument('--config-path', default='../../packages/planning_node/config.json')
    parser.add_argument('--new-config-path', default='../../packages/planning_node/new_config.json')
...
def main(args: argparse.Namespace):
    ...
    if circle_count > 1:
        ...
        print('Given spacing: {:.3f}, Actual spacing: {:.3f}'.format(circle_spacing, actual_spacing))
        ...
        for i in range(circle_count):
            circles.append({
                'center': {
                    'x': min_side / 2 + (max_side - min_side) * i / (circle_count - 1),
                    'y': min_side / 2,
                },
                'radius': radius,
            })
