...
std::vector<geom::Vec2> toVec2Array(const RTreeIndexedPoints& rtree_indexed_points) {
...
RTreeIndexedPoints RTreeQueryBox(const RTree& rtree, const geom::Vec2& point, double radius) {
    RTreeIndexedPoints rtree_indexed_points;

    RTreeBox rtree_box(
        RTreePoint(point.x - radius, point.y - radius),
        RTreePoint(point.x + radius, point.y + radius));
...
std::vector<geom::Vec2> buildRoutingMesh(
    ...
    auto addOrthogonalMeshFromPose = [&](const geom::Pose& pose) {
        ...
        for (size_t i = 1; i < half_points_count + 1; i++) {
            ...
            rtree_mesh_points.insert(toRTreeIndexedPoint(left, rtree_mesh_points.size()));
            rtree_mesh_points.insert(toRTreeIndexedPoint(right, rtree_mesh_points.size()));
        }
    };
    ...
    return toVec2Array(RTreeQueryBox(rtree_mesh_points, ego, params.border_radius));
}
