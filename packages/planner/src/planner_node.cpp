...
PlannerNode::PlannerNode() : Node("planner") {
    ...
    search::SearcherParams searcher_params{
        ...
        .max_node_position_error =
            this->declare_parameter<double>("searcher.max_node_position_error")
    };
...
void PlannerNode::onFinishPoint(const geometry_msgs::msg::PointStamped::SharedPtr msg) {
    ...
    if (!msg->header.frame_id.empty() && msg->header.frame_id != target) {
        const auto tf_opt = getLatestTranform(msg->header.frame_id, target);
        if (!tf_opt) {
            RCLCPP_WARN(
                this->get_logger(),
                "Can't lookup transform from '%s' to '%s', ignore finish point!",
...
void PlannerNode::publishGrid() const {
    ...
    marker.header.frame_id = state_.odom ? state_.odom->header.frame_id : "odom_ekf";
    marker.id = 0;
...
void PlannerNode::doPlanningLoop() {
    if (!edge_cache_ || !checker_->initialized() || !state_.ego_pose.has_value()
