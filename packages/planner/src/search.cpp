...
bool Grid::build() {
    ...
    for (size_t i = 0; i < params_.height; i++) {
        for (size_t j = 0; j < params_.width; j++) {
            const auto point = origin_clipped + (geom::Vec2(j, i) * params_.resolution);
            Node node = Node{
                .index = (i * params_.width) + j,
                .point = point,
                .collision = checker_->distance(point) < (shape_.width / 2)};
...
size_t EdgeCache::getYawIndexFromAngle(double theta) const {
    ...
    if (index == edge_params_.yaws_count) {
        index = 0;
    }
...
void PrimitiveCache::parseJSON() {
    ...
    const auto& indices =
        data.contains("primitives_indices") ? data["primitives_indices"]
                                            : data["yaw_to_primitives_indexing"];
    edges_indices_ = indices.get<std::vector<std::vector<size_t>>>();
    ...
    for (json::iterator it = data["primitives"].begin(); it != data["primitives"].end(); it++) {
        ...
        for (size_t i = 0; i < x.size(); i++) {
            poses.push_back({geom::Vec2(x[i], y[i]), geom::AngleVec2(geom::Angle(theta[i]))});
        }
...
void Searcher::setCollisionChecker(
...
bool Searcher::collisionFree(const EdgeInfo& edge) const {
    ...
    for (const geom::Pose& pose : edge_cache_->getPosesByEdgeIndex(edge.index)) {
        if (checker_->distance({pose.pos + origin, pose.dir}) < params_.min_obstacle_distance) {
            return false;
        }
    }
...
bool Searcher::insideBorders(const EdgeInfo& edge) const {
    ...
    geom::Vec2 nearest_node_pos =
        grid_->getNodeByIndex(grid_->getNodeIndexByPoint(edge_global_finish_pos)).point;
...
void Searcher::State::addEdge(const geom::Poses& edge, const geom::Vec2& origin) {
    ...
    for (const geom::Pose& pose : edge) {
        geom::Pose global_pose{pose.pos + origin, pose.dir};
...
geom::Poses Searcher::State::constructPath(const geom::Pose& ego) const {
    ...
    for (size_t i = 0; i < edges.size(); i++) {
        ...
        for (size_t j = 0; j < max_j; j++) {
            rtree_path.insert(
                std::make_pair(Point(edges[i][j].pos.x, edges[i][j].pos.y), path.size()));
...
void Searcher::buildState() {
    ...
    while (prev_vertex_exist) {
        ...
        geom::Poses edge_poses =
            edge_cache_->getPosesByEdgeIndex(current_vertex.state.prev_edge_index.value());
