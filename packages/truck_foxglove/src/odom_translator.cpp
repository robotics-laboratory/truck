...
OdometryTranslator::OdometryTranslator()
    ...
    RCLCPP_INFO(
        this->get_logger(), "Minimal difference between odometry markers: %f", _distSensitivity);
...
#pragma GCC diagnostic ignored "-Wunused-result"
    ...
    _pathPublisher = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        _pathTopic, rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(_odomQoSProfile)));

    _curPublisher = this->create_publisher<visualization_msgs::msg::Marker>(
        _curTopic, rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(_odomQoSProfile)));
...
void OdometryTranslator::republishOdometryPath() {
    if (_latest != nav_msgs::msg::Odometry() && checkOdomDiffSignificance()) {
        ...
        if (_markers.size() > 0 && _markers.size() > _arraySize) _markers.erase(_markers.begin());
...
bool OdometryTranslator::checkOdomDiffSignificance() {
    if (_markers.size() == 0) return true;
