...
void PlannerVisualizationNode::publishGraph() {
    ...
    std::string log = "S: " + std::to_string(nodes_.size());
    ...
    log = "Size #1: " + std::to_string(pointcloud_base.points.size()) +
                      ", Size #2: " + std::to_string(pointcloud_accent.points.size());
    RCLCPP_INFO(this->get_logger(), log.c_str());

    pcl::toROSMsg(pointcloud_base, pointcloud2_base);
    pcl::toROSMsg(pointcloud_accent, pointcloud2_accent);

    log = "Size #3: " + std::to_string(pointcloud2_accent.data.size()) +
          ", Size #4: " + std::to_string(pointcloud2_base.data.size()) + "\n";
    RCLCPP_INFO(this->get_logger(), log.c_str());
...
void PlannerVisualizationNode::timerCallback() {
    ...
    std::vector<search::Node> nodes_ = builder.build();
