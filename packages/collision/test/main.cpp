...
TEST(CollisionMap, BitMap) {
...

    {
        ...
        EXPECT_EQ(bit_map[0][0], 1);
        EXPECT_EQ(bit_map[0][1], 1);
        EXPECT_EQ(bit_map[0][2], 1);
        EXPECT_EQ(bit_map[1][0], 1);
        EXPECT_EQ(bit_map[1][1], 0);
        EXPECT_EQ(bit_map[1][2], 0);
        EXPECT_EQ(bit_map[2][0], 1);
        EXPECT_EQ(bit_map[2][1], 0);
        EXPECT_EQ(bit_map[2][2], 0);
    }
...

    {
        ...
        ComplexPolygon polygon;
        polygon.outer = {Vec2(-1, -1), Vec2(-1, 5), Vec2(5, 5), Vec2(5, -1)};
        polygon.inners = {
            {Vec2(-eps, -eps), Vec2(-eps, 1), Vec2(1, 1), Vec2(1, -eps)},
            {Vec2(2 - eps, 2 - eps), Vec2(2 - eps, 3), Vec2(3, 3), Vec2(3, 2 - eps)}};
...
TEST(CollisionMap, DistanceMap) {
    ...
    ComplexPolygon polygon;
    polygon.outer = {Vec2(-1, -1), Vec2(-1, 5), Vec2(5, 5), Vec2(5, -1)};
    polygon.inners = {{Vec2(2, 2), Vec2(2, 3 - eps), Vec2(3 - eps, 3 - eps), Vec2(3 - eps, 2)}};
