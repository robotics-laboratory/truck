...
namespace truck::nav2_plan_follower {
...
Nav2PlanFollowerNode::Nav2PlanFollowerNode() : Node("nav2_plan_follower") {
    ...
    service_.reset = this->create_service<std_srvs::srv::Empty>(
        "/reset_path", std::bind(&Nav2PlanFollowerNode::onReset, this, _1, _2));

    slot_.odometry = this->create_subscription<nav_msgs::msg::Odometry>(
        "/ekf/odometry/filtered", 1, std::bind(&Nav2PlanFollowerNode::onOdometry, this, _1));

    slot_.grid = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
        "/grid", 1, std::bind(&Nav2PlanFollowerNode::onGrid, this, _1));

    using TfCallback = std::function<void(tf2_msgs::msg::TFMessage::SharedPtr)>;

    const TfCallback tf_call = std::bind(&Nav2PlanFollowerNode::onTf, this, _1, false);
    const TfCallback static_tf_callback = std::bind(&Nav2PlanFollowerNode::onTf, this, _1, true);
    ...
    timer_.main = this->create_wall_timer(
        params_.period, std::bind(&Nav2PlanFollowerNode::publishFullState, this));
...
void Nav2PlanFollowerNode::publishFullState() {
    publishTrajectory();
    publishGridCostMap();
}
...
void Nav2PlanFollowerNode::publishGridCostMap() {
    if (!state_.distance_transform) {
        return;
    }
...
void Nav2PlanFollowerNode::onOdometry(nav_msgs::msg::Odometry::SharedPtr odometry) {
    state_.localization = geom::toLocalization(*odometry);
    state_.odometry = odometry;
}

void Nav2PlanFollowerNode::onGrid(nav_msgs::msg::OccupancyGrid::SharedPtr msg) {
    if (!state_.odometry) {
        return;
    }
...
void Nav2PlanFollowerNode::onTf(tf2_msgs::msg::TFMessage::SharedPtr msg, bool is_static) {
...
std::optional<geom::Transform> Nav2PlanFollowerNode::getLatestTranform(
