...
ICP ICPBuilder::build() {
    ...
    std::shared_ptr <Matcher::ErrorMinimizer> point_to_plane = \
        Matcher::get().ErrorMinimizerRegistrar.create("PointToPlaneErrorMinimizer", point_to_plane_params_params);
    
    std::shared_ptr <Matcher::OutlierFilter> robust_outlier_filter = \
        Matcher::get().OutlierFilterRegistrar.create("RobustOutlierFilter", robust_outlier_filter_params);
