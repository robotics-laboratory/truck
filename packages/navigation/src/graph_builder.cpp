    ...
    rtree.query(
        bg::index::intersects(rtree_box)
            && bg::index::satisfies([&](RTreeIndexedPoint const& rtree_indexed_point) {
                   geom::Vec2 const neighbor_point = toVec2(rtree_indexed_point.first);
...
std::vector<NodeId> getNodeNeighbors(
    ...
    for (const RTreeIndexedPoint& rtree_indexed_point : rtree_indexed_points) {
        NodeId const neighbor_node_id = rtree_indexed_point.second;
        geom::Vec2 const neighbor_node_point = toVec2(rtree_indexed_point.first);

        geom::Segment const edge(node.point, neighbor_node_point);
...
Graph GraphBuilder::build(
    ...
    RTree const rtree_nodes = toRTree(graph.nodes);

    EdgesMap nodes_to_edges;

    for (Node const& cur_node : graph.nodes) {
        for (NodeId const neighbor_node_id :
